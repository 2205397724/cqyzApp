<template>
	<view style="padding-bottom: 100rpx;">
		<headbar fixed title="违建详情" rightIcon="redo" @clickRight="clickRightFunc"></headbar>
		<mescroll-body ref="mescrollRef" :down="downOption" :up="upOption" @init="mescrollInit" @down="downCallback" @up="upCallback">
			<view class="na-header-white">
				...
				<view class="flex-row p-20 size-sm" style="flex-wrap: wrap;padding-bottom: 0;">
					<view>
						<text class="font-grey p-r-10">状态</text>
						<text class="na-tag2">{{getFuncOptVal(opts_all.toushu2_status,details.status) || details.status==0?'未处理':details.status==1?'已审核':'' }}</text>
					</view>
					<view class="m-r-20">
					...
					<!-- <view>
						<text class="font-grey p-r-10">评分</text>
						<text class="">85</text>
					</view> -->
				</view>
			</view>
			<view class="na-main">
				<view class="na-title p-t-10">
					<text class="text">违建信息</text>
				</view>
				<view class="na-card p-30 bg-white">
					<view class="line-height-170">
						<view class="flex-row flex-start">
							<text class="size-sm font-grey">发起人：</text>
							<text class="size-base font-darkgrey" v-if="details.uinfo">{{details.uinfo.name || details.uinfo.nickname || details.uinfo.username}}</text>
						</view>
						<view class="flex-row flex-start">
							<text class="size-sm font-grey">发起人电话：</text>
						...
						<view class="flex-column">
							<view class="size-sm font-grey">回复内容：</view>
							<view class="size-lg">{{details.content}}</view>
						</view>
			...
			<view class="na-main">
				...
				<view class="na-card m-tb-10 bg-white">
					<view class="na-steps-column m-l-40 select" v-for="(item,i) in details.totlogs">
						<view class="na-steps-column-item m-b-10">
							<view class="steps-item-discription">
								<text>{{ item.nickname || "匿名用户"}}</text>
								<text class="m-l-20 size-base strong">{{item.content}}</text>
								<text>时间：{{item.created_at}}</text>
							</view>
							<view class="flex-column na-photo-box" v-if="item.logable.content">
								<view>{{item.logable.content}}</view>
								<view class="photo-item" v-for="(src,i) in item.logable.affixs">
									<view class="photo-item-image">
			...
			<view class="na-bottom-tool bg-white">
				<view class="p-tb-20 p-lr-30 flex-row">
					<!-- <view class="btn-primary btn-size-sm btn-block m-r-10" @tap="clicktype('score')">评分</view> -->
					<view class="btn-primary btn-size-sm btn-block m-r-10" @tap="clicktype('complaintplus')">追问</view>
				</view>
			</view>
		</mescroll-body>
		<!-- ====================弹窗部分===================== -->
...
<script>
	...
	export default {
		...
		onLoad(opts) {
			this.id = opts.id
			getOpts(['toushu2_status']).then(res => {
				this.opts_all = res
			})
		},
		methods: {
