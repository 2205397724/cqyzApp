<template>
	<view style="padding-bottom: 100rpx;">
		<headbar fixed title="物业保修" rightIcon="redo" @clickRight="clickRightFunc"></headbar>
				...
				<view class="na-main p-t-30">
					...
					<view class="flex-row p-tb-20 size-sm" style="flex-wrap: wrap;padding-bottom: 0;">
						<view>
							...
							<text :class="details.status === 0 ? 'na-tag2' :'na-tag1'" >{{getFuncOptVal(opts_all.repair_status,details.status)}}</text>
						</view>
						<view class="m-r-20">
		...
		<view class="na-bottom-tool bg-white">
			<view v-if="details.uid == user_info.id">
				<view class="p-tb-20 p-lr-30 flex-row" v-if="details.status>4 && details.status<99">
...
<script>
	...
	export default {
		...
		onLoad(opts) {
			this.id = opts.id
			getOpts(['repair_status']).then(res => {
				this.opts_all = res
			})
