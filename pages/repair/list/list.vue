	...
	<view>
		<view class="na-header-white">
			...
			<uni-row class="na-main p-tb-20 flex-row flex-center">
				<uni-col :span="18">
					<view class="size-lxx strong">报修</view>
					<view class="size-base p-t-10 font-grey">{{ zone_detail.name }} 所有报修</view>
				</uni-col>
				<uni-col :span="6" class="flex-row flex-end">
					<view class="btn-size-xs btn-extra font-primary" @tap="add()">发起报修</view>
				</uni-col>
			</uni-row>
		</view>
					...
					<uni-row class="na-list-item na-card bg-white p-30">
						<uni-col :span="24">
							<view class="list-content" @tap="details(item.id)">
								...
								<view class="p-tb-20">
									<!-- <text class="na-tag2" >{{getFuncOptVal(opts_all.repair_status,item.status)}}</text> -->
									<text :class="item.status === 0 ? 'na-tag2' :'na-tag1'" >{{getFuncOptVal(opts_all.repair_status,item.status)}}</text>
								</view>
								<view class="list-info">
									<text v-if="item.uinfo">报修人：{{item.ano?'匿名业主':(item.uinfo.nickname || '匿名业主')}}</text>
									<text>报修时间：{{item.created_at}}</text>
								</view>
							</view>
						</uni-col>
					</uni-row>
...
<script>
	...
	export default {
		...
		onLoad(opts) {
			...
			getOpts(['is_illegal','repair_status']).then(res => {
				this.opts_all = res
			})
		},
		methods: {
			...
			details(id){
				this.utils.route('/pages/repair/detail/detail?id='+id)
			},
			add(){
				this.utils.route('/pages/repair/add/add')
			},
