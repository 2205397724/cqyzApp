	...
	<view class="explain-form-control">
		...
		<uni-forms-item v-else-if="['upload'].includes(schema.type)" :name="schema.name" :label="schema.label"
			...
			<uni-file-picker ref="uploadFilePicker" v-model="val" :title="schema.title" :limit="schema.limit"
				:file-mediatype="schema.fileType||'image'" @progress="onProgress" @success="onSuccess" @fail="onFail"
		...
		<view class="explain-form-tips" v-if="schema.description">
	...
	export default {
		...
		created() {
			...
			if (type === 'picker-city-china') {
				...
				this.val = this.evalExpr(schema.value)
			}
			if (schema.description) {
				this.desc = this.evalExpr(schema.description)
			}
			this.$emit('created', {
				schema
			})
		...
		methods: {
			...
			onSelect(e) {
				...
				if (type === 'upload') {
					// 选择文件
					if(value){
						value = value.concat(e.tempFiles)
					}else{
						value = e.tempFiles
					}
					this.$emit('output', {
			...
			renderTpl() {
				this.val = this.evalExpr(this.schema.value)
			},
