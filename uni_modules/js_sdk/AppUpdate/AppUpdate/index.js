...
import config from '@/pages/framework/config/app.js'
const platform = uni.getSystemInfoSync().platform;
// 主颜色
const $mainColor = "00d5c0";
// 弹窗图标url
const $iconUrl = '/static/updatebg.png';
...
const getServerNo = function(version, isPrompt = false, callback) {
	...
	let resdata = {};
	//检测版本名称，小版本增量更新，大版本整包更新。
	const shouldWgt = function(versionname, versionnameLocation) {
		let v = versionname.split(".");
		let v_Location = versionnameLocation.split(".")
		return (v[0] * 1 == v_Location[0] * 1) &&
				(v[1] * 1 == v_Location[1] * 1) && 
				(v[2] * 1 > v_Location[2] * 1)
	}
	let param = {
		appid : config.APPID,
		status : 1
	...
	if (platform == "android"){
		param.type = 1//安卓
		request.GET('appversion',param).then((res) => {
			resdata = res.data[0]
			if(resdata.version*1 > httpData.version*1){
				httpData.type = 1101;
				if (isPrompt == true) {
					let downloadUrl = resdata.url
					if (shouldWgt(resdata.version_name, httpData.versionName)) {
						downloadUrl = resdata.url
					}
					callback && callback({
						versionCode: resdata.version,
						versionName: resdata.version_name,
						versionInfo: resdata.content,
						forceUpdate: resdata.force,
		...
		request.GET('appversion',param).then((res) => {
			...
			if(resdata.version*1 > httpData.version*1){
				...
				if (isPrompt == true) { 
					let downloadUrl = '';
					let appstoreUrl = resdata.url
					//后端接口数据 updatetype为101整体更新，102增量更新。增量更新时就下载资源，否则跳转到appstore
					if (shouldWgt(resdata.version_name, httpData.versionName) && resdata.updatetype == 102) {
						downloadUrl = resdata.url
						appstoreUrl = ''
					}
					callback && callback({
						versionCode: resdata.version, //resdata.version,
						versionName: resdata.version_name,
						versionInfo: resdata.content,
						forceUpdate: resdata.force,
			...
			if (res.forceUpdate) { //强制更新
				if (/\.wgt$/i.test(res.downloadUrl)) {
					...
					if (platform == "android") {
						...
						//如果有下载地址，则下载，无下载地址跳转appstore
						if (res.downloadUrl) {
							getDownload(res);
						} else {
							plus.runtime.openURL(res.appstoreUrl);
						}
					}
				}
			} else {
