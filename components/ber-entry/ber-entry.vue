<template>
	<view>
		<view>
			<button type="primary" style="margin-bottom: 20rpx;" @click="clickFuncAdd">添加报名事件</button>
		</view>
		<view>
			<!-- 禁止滑动 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 添加表决事件
			clickFuncAdd(){
				this.utils.route(`/pages/survey/entry_add/entry_add`)
			},
			// 修改表决事件
			clickFuncDetails(id){
				this.utils.route(`/pages/survey/entry_add/entry_add?opt=${encodeURIComponent(JSON.stringify(id))}`)
			},
